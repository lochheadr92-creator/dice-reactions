...
- **Blocker resolution path (proposed, not yet implemented):** ADR-025 (`docs/adr-025-action-duration-authority.md`, Proposed 2026-07-04) drafts the owner-ratified v1 policy — ordinary actions advance 0 days; only an explicit structured `rest`/`wait`/`travel` time command advances time (engine-validated, engine-produced event as sole authority); same-turn post-acceptance integration committed in the existing CAS update; sub-day = 0 days, no rounding, no accumulation. The blocker stays OPEN until ADR-025 is accepted and its implementation task lands with the brief §12 test plan.
