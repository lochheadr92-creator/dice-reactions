...
| Replayability Engine v1 | Verified complete (contract-corrected — session document only) | `replayability.py`, `run_identity.py`, `opening_state.py`, `pressure_graph.py`, `consequence_echoes.py`, `server.py` | Five replayability test modules ✅ (~97 cases in modules; 185 targeted bundle) | `decision-log.md` (ADR-019), `story-engine.md` | Echoes require confirmed structured sources; no player-text parsing; causal pressure graph; `transition_receipts` only | Theft/violence/promise echoes until guard outputs; Living World Test |
