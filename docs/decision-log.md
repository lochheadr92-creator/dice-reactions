...
| **Evidence update** | Deterministic backend proof now covers feature-flag handoff, eligible candidate generation, agreement and disagreement paths, authorised Utility AI winners, and committed NPC move receipts matching the Utility AI winner. Dev-only live-selection diagnostics are available. This does not promote `TURN_INTEGRATION_VERIFIED`; live LLM gameplay validation remains a separate acceptance step. |
...
| Field | Detail |
|-------|--------|
| **Date** | Proposed 2026-07-04; owner amendments ratified and **Accepted 2026-07-04** |
| **Status** | **Accepted** — documentation-only ADR (no code/test changes). Blocker `D_ACTION_DURATION_AUTHORITY` stays OPEN until the Phase 2A runtime implementation and its tests land and are verified. |
...
| **Decision** | (1) Ordinary actions advance exactly 0 days — no category→duration table in v1. (2) Time advances only via an explicit structured `time_command` (family `wait`/`rest`/`travel`); the engine owns command validation, approved duration, event construction, and clock mutation — the engine event, never the client value, is the authority; the client must not provide the event ID. (3) Corrected same-turn pipeline: validate command → produce `pending_time_advance` on the in-memory working state → `prepare_action_turn` consumes it through the existing clock seam BEFORE provider narration → provider narrates against updated authoritative working state → parse/guard → advance becomes authoritative only when the existing `_persist_story_action_turn` CAS succeeds. (4) `MAX_TIME_COMMAND_DAYS = 30` approved as a hard safety ceiling (not a default); requests <1 or >30 whole days are rejected — never silently clamped; the constant must live in the authoritative constants/configuration location. (5) Sub-day: 0 days, no rounding, no accumulation; optional explicit zero-day audit events only when genuinely required for traceability; fractional clock deferred to a schema change. |
| **Phase 2A scope** | First runtime implementation enables ONLY `WAIT_ONE_DAY` → 1 approved simulation day. `rest`, `travel`, arbitrary N-day commands, sub-day accumulation, and broad action-duration inference remain deferred. |
| **Idempotency** | Deterministic engine-constructed `event_id = time_advance:{session_id}:{turn_number}:{command_kind}:{approved_days}` (lease-stable across retries); at most one persisted advance per accepted action; refuse-overwrite of an existing pending event; permanent replay barrier via `expected_previous_simulation_day`. Any failure (provider, parsing, validation, lease ownership, CAS) persists no advance; a retry sees the previously persisted clock and safely recreates the same deterministic event. |
| **Gating** | Separate flag `ENABLE_TIME_COMMANDS` (default OFF) gates command acceptance and authoritative clock advancement; `ENABLE_NPC_LIFECYCLE` (default OFF) gates NPC aging/mortality/lifecycle after an advance. Clock progression must not conceptually depend on NPC lifecycle being enabled. Flag OFF rejects `time_command` deterministically (never silently ignored); default deployed behaviour byte-identical to today. |
| **Rejected** | Category duration tables (no approved source of truth); LLM/parser-derived duration (violates state authority + determinism); trusting or silently clamping client values; post-guard event production (provider would narrate against a stale clock — owner-corrected to pre-provider); next-turn staged consumption (visible cause/effect mismatch); sub-day rounding/accumulation (schema migration deferred). |
